---
title: 'Code Supplement - Allbee et al. xxxx'
...
setwd("~/Dropbox/Academics/Research Projects/Oakridge_nearfar/manuscript/final analysis - code supplement")
...
######
#
# Figure S2 - density plot of movement distance of all species by herbivore treatment
#
######

#pdf(file="Fig_S2_herbivorekernels.pdf",width=7, height=7)
...
######
#
# GLMER analysis of number of established stems ~ treatment * species
#
######
...
######
#
# Figure 3 - abundance of 4 target species per treatment
#
######
...
#pdf(file="Fig_3_stemabundance.pdf", width=10, height=7)
...
Here we examine the alpha diversity of all moved core species across all treatments (distance, competition (covered/uncovered), herbivores). 

RESULTS: More core species moved to near plots as opposed to far, covered plots had more core species than uncovered plots, and herbivores did not have an effect on the diversity of core species within the plots.
...
#wide 
moved_cover_ord <- spp_level_move[,c(1,5:9)] %>%
  spread(key = sixletter_code, value = percent_cover, fill = 0)

#merge in treatment data
high_div_move_alpha <- merge(high_div_move_alpha, trt, by=c("plot", "transect"))
...
######
#
# LMER analysis of alpha diversity ~ dispersal * herbivory * competition
#
######
...
######
#
# Figure 4 - alpha diversity with competition and herbivory treatments
#
######
...
stats_alpha$cover_status <- factor(stats_alpha$cover_status, levels = c("cover", "uncover"),
                      labels = c("no", "yes"))



#pdf(file="Fig_4_alpha.pdf", width=7, height=7)

ggplot(stats_alpha, aes(x=cover_status, y=mean_alpha, shape = distance))+
                geom_errorbar(aes(ymax = mean_alpha + se_alpha, ymin = mean_alpha - se_alpha, 
                                  color = distance), width = 0.15, lwd = 1.75)+
                geom_point(cex = 7)+
                theme_bw()+
                theme(aspect.ratio=1)+
                scale_color_manual(values = c("black", "black"))+
                labs(x="Competition via Priority Effects",y="Alpha Diversity of Moved Species")+
...
adonis(dat.jaccard.moved ~ herbivores + cover_status + distance , data = dat_env,
       strata = dat_env$plot)
...
scores_moved2 <- cbind(scores_moved, dat_env)
...
######
#
# Figure 5 - species trade-offs between dispersal, herbivory tolerance and competition ability
#
######
